...
func CreateFseq(af AudioFile) (f *Fseq) {
    f = newFseq()
    f.fdetect(af)
    f.pdetect(af, 55.0, 880.0)
    return
}
...
func (f *Fseq) pdetect(af AudioFile, lower, upper float64) {
    ...
    for index := uint(0); index < af.Length(); index += width*2 {
        samps, err := af.GetSamplesAt(index, width*2)
        if err != nil {
            panic(err)
        }
        bestComb := uint(0)
...
func (f *Fseq) fdetect(af AudioFile) {
    s := Analyze(af)
...
func pickStoreFormants( f *Fseq, i, count int, vpowers, vfreqs, upowers, ufreqs, bandFreqs, vuratios []float64) {
    ...
    for v := 0; v < VoicedOps; v++ {
        f.Voiced[i][v] = OperatorFrame{ vpowers[bestIndexes[v]], vfreqs[bestIndexes[v]] }
        f.Unvoiced[i][v] = OperatorFrame{ upowers[bestIndexes[v]], ufreqs[bestIndexes[v]] }
    }
}