...
func sampsToComplex(s []float64) []_Ctypedef_complex8 {
	c := make([]_Ctypedef_complex8, len(s))
	for i, r := range s {
		c[i].re = _Ctypedef_real8(r)
	}
	return c
}
...
func Analyze(af AudioFile) (s *SpectralAnalysis) {
	...
	for f := uint(0); f < Frames; f++ {
		...
		for i := 0; i < FftBins; i++ {
			...
			c[i].re *= _Ctypedef_real8(hamming)
		}
		_Cfunc_fftc8_1024((*_Ctypedef_complex8)(unsafe.Pointer(&c[0])))
